...
namespace prototype::network {
    ...
    void handle_client(SOCKET client_fd, SSL_CTX* ssl_ctx, prototype::database::DatabaseManager* db) {
        ...
        while (true) {
            ...
            if (packet.header.type == PacketType::LOGIN_REQUEST || packet.header.type == PacketType::REGISTER_REQUEST) {
                ...
                if (success) {
                    if (!my_uuid.empty()) {
                        global_registry.force_disconnect_all(my_uuid);
                    }
                ...
                if (db->get_user_by_name(contact_name, c_user)) {
                    ...
                    RawPacket resp;
                    resp.header.type = PacketType::CONTACT_ADD;
        ...
        if (!my_uuid.empty()) global_registry.force_disconnect_all(my_uuid);
        closesocket(client_fd);
    }
}
...
int main() {
    ...
    std::string config_dir = std::string(appdata ? appdata : "C:") + "\\" + ALBO_APP_DIR;
    ...
    if (!std::filesystem::exists(config_dir + "\\server.crt")) {
        // We'll assume openssl is in path or skip if not found, but we need these files.
        // For now, if they don't exist, this will likely fail unless generated.
    ...
    if (SSL_CTX_use_certificate_file(ssl_ctx, (config_dir + "\\server.crt").c_str(), SSL_FILETYPE_PEM) <= 0 ||
        ...
        std::cerr << "Failed to load certificates from " << config_dir << "\n";
        return 1;
    }

    prototype::network::ConfigManager config(config_dir + "\\server.conf"); config.load();
    
    std::string db_path = config.get("db_path", config_dir + "\\albo.db");
    prototype::database::DatabaseManager db(db_path);
    db.initialize();
    ...
    int opt = 1; setsockopt(s_fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&opt, sizeof(opt));
    
    sockaddr_in addr{}; 
    addr.sin_family = AF_INET; 
    addr.sin_addr.s_addr = INADDR_ANY; 
    addr.sin_port = htons(config.get_int("port", ALBO_DEFAULT_PORT));

    if (bind(s_fd, (struct sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
        std::cerr << "Bind failed: " << WSAGetLastError() << "\n";
        return 1;
    }
    listen(s_fd, 10);

    ALBO_LOG("ALBO Messenger Server Started on port " << config.get_int("port", ALBO_DEFAULT_PORT));

    while (true) {
        sockaddr_in c_addr{}; int len = sizeof(c_addr);
        SOCKET c_fd = accept(s_fd, (struct sockaddr*)&c_addr, &len);
